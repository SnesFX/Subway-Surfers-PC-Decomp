...
public static class NGUIMath
{
	...
	public static Bounds CalculateAbsoluteWidgetBounds(Transform trans)
	{
		...
		for (int num = componentsInChildren.Length; i < num; i++)
		{
			...
			Vector3 vector = cachedTransform.TransformPoint(new Vector3(num2 - relativeSize.x, num3 - relativeSize.y, 0f));
			if (flag)
			{
				flag = false;
			...
			result.Encapsulate(cachedTransform.TransformPoint(new Vector3(num2 - relativeSize.x, num3 + relativeSize.y, 0f)));
			result.Encapsulate(cachedTransform.TransformPoint(new Vector3(num2 + relativeSize.x, num3 - relativeSize.y, 0f)));
			result.Encapsulate(cachedTransform.TransformPoint(new Vector3(num2 + relativeSize.x, num3 + relativeSize.y, 0f)));
		}
		return result;
